...
class DevRunner:
    ...
    def demo(self):
        """Run interactive demo showcasing textual-snapshots features"""
        self.log("🎭 Running textual-snapshots demo...")

        examples_dir = self.project_root / "examples"
        if not examples_dir.exists():
            self.error("Examples directory not found. Run from project root.")
            return

        # First, show the conceptual demo
        simple_demo = examples_dir / "simple_demo.py"
        if simple_demo.exists():
            self.log("📖 Showing what textual-snapshots does...")
            try:
                result = subprocess.run([sys.executable, str(simple_demo)],
                                      capture_output=True, text=True, timeout=30)
                if result.returncode == 0:
                    print(result.stdout)
                else:
                    self.warning(f"Simple demo failed: {result.stderr}")
            except subprocess.TimeoutExpired:
                self.warning("Simple demo timed out")
            except Exception as e:
                self.warning(f"Simple demo error: {e}")

        # Ask if user wants to see working examples
        try:
            response = input("\n🤔 Want to see working examples? (y/N): ").strip().lower()
            if response in ['y', 'yes']:
                self._run_working_examples(examples_dir)
            else:
                self.log("👍 Skipping working examples")
    ...
    def _run_working_examples(self, examples_dir):
        """Run the actual working examples."""
        # Run basic capture example
        basic_example = examples_dir / "basic_capture.py"
        if basic_example.exists():
            self.log("📸 Running basic capture example...")
            try:
                self.uv_run(['python', str(basic_example)])
            except subprocess.CalledProcessError as e:
                self.warning(f"Basic example failed: {e}")
                self.log("💡 This might be due to missing dependencies or setup issues")

        # Show what was created
        screenshots_dir = self.project_root / "screenshots"
        if screenshots_dir.exists():
            screenshots = list(screenshots_dir.glob("*.svg"))
            if screenshots:
                self.log(f"📁 Created {len(screenshots)} screenshots:")
                for screenshot in screenshots[-3:]:  # Show last 3
                    size = screenshot.stat().st_size
                    self.log(f"  • {screenshot.name} ({size:,} bytes)")

                # Ask if user wants to open a screenshot
                try:
                    response = input("\n🖼️  Open a screenshot in browser? (y/N): ").strip().lower()
                    if response in ['y', 'yes'] and screenshots:
                        import webbrowser
                        webbrowser.open(f"file://{screenshots[0].absolute()}")
                        self.log(f"🌐 Opened {screenshots[0].name} in browser")
                except KeyboardInterrupt:
                    pass
        ...
        try:
            result = subprocess.run(['uv', 'run', 'textual-snapshot', '--help'],
                                  capture_output=True, text=True, timeout=10)
            if result.returncode == 0:
                self.log("  ✅ CLI help working")
            else:
                self.warning("  ❌ CLI help failed")
        except Exception:
            self.warning("  ❌ CLI help failed")

        # Show next steps
        self.log("\n🚀 Next steps:")
        self.log("  • Check screenshots/ directory for captured images")
        self.log("  • Open .svg files in your browser to view them")
        self.log("  • Run 'python examples/plugin_example.py' for advanced features")
        self.log("  • Try 'textual-snapshot --help' to explore CLI options")
        self.log("  • Read GETTING_STARTED.md for detailed tutorials")
